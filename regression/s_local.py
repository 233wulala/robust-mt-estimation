...
from resistics.regression.compute import spectralMatrices, saveImpedanceCSV, saveSpectralCSV
...
from resistics.regression.robust import eps, weightLS
...
class LocalRegressor(ResisticsBase):
    ...
    def process(self, ncores: int = 0) -> None:
        ...
        for declevel in range(0, numLevels):
            ...
            # #####⭐调用输出csv文件的函数
            # out1 = out
            # saveSpectralCSV(out1, evalFreq, declevel)
            # ##############
            ...
            for eIdx in range(0, numEvalFreq):
                ...
                out, var = self.robustProcess(numSolveWindows, obs, reg, evalFreq[eIdx])
            ...
            ####⭐新增保存估计后的阻抗
            # saveImpedanceCSV(self.impedances, self.evalFreq)
    ...
    def robustProcess(
            self, numWindows: int, obs: np.ndarray, reg: np.ndarray, evalFreq: np.ndarray
    ) -> Tuple[np.ndarray, np.ndarray]:
        """Simplified robust regression processing

        输出结果：
        | 输出变量             | 含义          | 对应阻抗张量部分                         |
| ---------------- | ----------- | -------------------------------- |
| `output[i]`      | 回归得到的参数（复数） | 第 i 个电场对应的阻抗行（Zxx,Zxy 或 Zyx,Zyy） |
| `varOutput[i]`   | 参数方差估计      | 各阻抗分量的不确定度                       |
| `residsSq[i, :]` | 每个频点的残差平方   | 表征拟合误差，用于计算 RMS                  |


        | 输出通道 | 输入通道     | 求得的参数      | 对应张量行 |
| ---- | -------- | ---------- | ----- |
| Ex   | [Hx, Hy] | [Zxx, Zxy] | 第一行   |
| Ey   | [Hx, Hy] | [Zyx, Zyy] | 第二行   |


        | 项目 | 名称         | 在代码中的变量        | 维度              | 示例                          |
| -- | ---------- | -------------- | --------------- | --------------------------- |
| 输入 | 磁场（Hx, Hy） | `reg[i, :, :]` | (numWindows, 2) | `[Hx(f), Hy(f)]`            |
| 输出 | 电场（Ex, Ey） | `obs[i, :]`    | (numWindows,)   | `Ex(f)` 或 `Ey(f)`           |
| 结果 | 阻抗张量Z      | `output[i]`    | (2,)            | `[Zxx, Zxy]` 或 `[Zyx, Zyy]` |

        outSize = 2（Ex、Ey）
        inSize = 2（Hx、Hy）

        numWindows = 频点数（每个频率窗口）

        Streamlined version focusing on essential S-estimator functionality.
        """
        crossSize = len(self.crossChannels)
        ...
        for i in range(0, self.outSize):  # 对输出通道 ex、ey
            observation = obs[i, :]  # obs[i, :]  ≈ Ex(f) 或 Ey(f) 每个输出通道对应一个观测序列（频率点上电场复值）
            predictors = reg[i, :, :]  # reg[i, :, :]  ≈ [Hx(f), Hy(f)] 即在第 i 个输出通道（Ex 或 Ey）对应的所有频点上，输入变量是磁场 Hx、Hy。

            # S-estimator with fewer starting points for speed
            params, resids, scale, weights = sestimateModel(
                predictors, observation, intercept=self.intercept, nstarts=5
            )

            # #####⭐新增得到每个窗的权重
            # weights1 = weights.reshape(numWindows, crossSize)
            # window_weights = np.mean(weights1, axis=1)
            # # window_weights = np.median(weights1, axis=1)
            # #####⭐将权重值直接保存到文件中
            # # 创建保存目录
            # os.makedirs("weights_out_s", exist_ok=True)
            #
            # # 阻抗名称
            # impedance_names = ["x", "y"]
            # imp_name = impedance_names[i]
            #
            # # 生成DataFrame
            # df = pd.DataFrame({
            #     "window": range(numWindows),
            #     "weight": window_weights
            # })
            #
            # # 文件名包含频率和阻抗
            # filename = f"weights_out_s/weights_{evalFreq:.6f}_{imp_name}_s_70_5.csv"
            #
            ...
            # Use scale estimate directly with a conservative correction
            # base_variance = scale * scale * 2.5  # Conservative efficiency loss factor
            base_variance = (np.abs(scale) **2 )* 2.5

            # Simple design matrix approach
            if n_obs > p_params:
                XTX = np.dot(hermitianTranspose(predictors_clean), predictors_clean)
                # Add small regularization
                XTX += eps() * np.trace(XTX).real / XTX.shape[0] * np.eye(XTX.shape[0])
                XTX_inv = np.linalg.inv(XTX)
                varOut = base_variance * np.real(np.diag(XTX_inv))
            else:
                # Fallback for underdetermined systems
                varOut = np.ones(p_params) * base_variance

            varOut = np.maximum(varOut, eps())

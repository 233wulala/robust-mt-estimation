...
class LocalRegressor(ResisticsBase):
    ...
    def robustProcess(
        ...
        """Robust regression processing using MM-estimator

        Uses MM-estimator (S-estimate + M-estimate) for better efficiency while
        maintaining high breakdown point.
        ...
        for i in range(0, self.outSize):
            ...
            # --- 用 MM 估计 ---
            params, resids, scale, weights = mmSestimateModel(
            ...
            # #####⭐将权重值直接保存到文件中
            # # 创建保存目录
            # os.makedirs("weights_out_mm", exist_ok=True)
            #
            # # 阻抗名称
            # impedance_names = ["x", "y"]
            ...
            #
            # # 文件名包含频率和阻抗
            # filename = f"weights_out_mm/weights_{evalFreq:.6f}_{imp_name}_mm_70_5.csv"
            #
            ...
            if self.intercept:
                out = params[1:]
                predictors_var = predictors[:, 1:]
            else:
                out = params
                predictors_var = predictors

            n_obs = len(observation)
            p_params = len(out)
            robust_scale_squared = scale * scale
            weighted_predictors, _ = weightLS(predictors_var, observation, weights)
            gram_matrix = np.dot(hermitianTranspose(weighted_predictors), weighted_predictors)
            trace_val = np.trace(gram_matrix).real
            if trace_val > eps():
                reg_factor = eps() * trace_val / gram_matrix.shape[0]
                gram_matrix += reg_factor * np.eye(gram_matrix.shape[0])
            varPred = np.linalg.inv(gram_matrix)
            efficiency_correction = 1.2
            finite_sample_correction = float(n_obs) / float(max(n_obs - p_params - 1, 1))
            varOut = (efficiency_correction * finite_sample_correction *
                      robust_scale_squared * varPred)
            varOut = np.diag(varOut).real
    ...
    def stackedProcess(self, data: np.ndarray) -> np.ndarray:
        ...
        for i in range(0, self.outSize):
            ...
            # save the output
            ##########################################s
            out, resids, scale, weights = mmSestimateModel(
                predictors, observation, intercept=self.intercept
            )
            if self.intercept:
                output[i] = out[1:]

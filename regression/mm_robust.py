...
A = nobs * nregressors
x = nregressors
这些函数的来源是稳健统计，Huber, 2009
一般来说，线性回归是#有观测值y和预测因子A
Y是多个观察/响应
X是自变量，是未知的
y是x => y = Ax的线性函数
Y = nobs
...
def sestimateModel(A: np.ndarray, y: np.ndarray, **kwargs) -> Tuple:
    ...
    leverageWeights = getRobustLocationWeights_S(Pdiag / leverageScale, "bisquare")
    ...
    for start in range(options["nstarts"]):
        ...
        for iteration in range(options["maxiter"]):
            ...
            # ✅ scale相对变化小于阈值时收敛
            if iteration > 0 and abs(scale - prev_scale) < 1e-6 * prev_scale:
                break
            ...
            weights = getRobustLocationWeights_S(std_resids, options["weights"]) * leverageWeights
        ...
        if final_scale < best_scale:
            ...
            best_weights = getRobustLocationWeights_S(resids / final_scale, options["weights"]) * leverageWeights
...
# def mmSestimateModel(A: np.ndarray, y: np.ndarray, **kwargs) -> Tuple:
#     """MM-estimate using S-estimate as initial stage
#
#     Combines S-estimate (high breakdown) with M-estimate (high efficiency).
#     This is similar to mmestimateModel but uses S-estimate for initialization.
#
#     Parameters
#     ----------
#     A : np.ndarray
...
#     intercept = options["intercept"]
#
#     # Stage 1: S-estimate for initial robust fit and scale
#     s_params, s_resids, s_scale, s_weights = sestimateModel(
#         A, y, weights="bisquare", intercept=True, nstarts=10
#     )  # ⭐ bisquare改huber
#
#     # Stage 2: M-estimate with fixed scale from S-estimate
#     initial_dict = {
#         "params": s_params,
#         "resids": s_resids,
#         "scale": s_scale
#     }
#
#     # Use bisquare weights for final M-estimate (high efficiency)
#     mm_params, mm_resids, mm_scale, mm_weights = mestimateModel(
#         A, y, weights="bisquare", initial=initial_dict, intercept=intercept
#     )  # ⭐ bisquare改huber
#
#     return mm_params, mm_resids, mm_scale, mm_weights
...
def mmSestimateModel(A: np.ndarray, y: np.ndarray, **kwargs) -> Tuple:
    ...
    # ★ 在外部统一添加截距列
    if intercept:
        A = np.hstack((np.ones((A.shape[0], 1), dtype="complex"), A))

    # 两阶段都用 intercept=False，避免重复添加
    s_params, s_resids, s_scale, s_weights = sestimateModel(
        A, y, weights="bisquare", intercept=False, nstarts=10
    )

    initial_dict = {"params": s_params, "resids": s_resids, "scale": s_scale}

    mm_params, mm_resids, mm_scale, mm_weights = mestimateModel(
        A, y,
        weights="bisquare",
        initial=initial_dict,
        scale=s_scale,      # 显式固定scale
        intercept=False
    )
...
#     # outDict["weights"] = "huber"  # ⭐use existing weight functions
#     outDict["weights"] = "bisquare"  # use existing weight functions
#     outDict["maxiter"] = 50  # fewer iterations for S-estimate
#     outDict["intercept"] = False
#     outDict["nstarts"] = 10  # reasonable number of starts
#     return outDict
...
def mestimateModel(A: np.ndarray, y: np.ndarray, **kwargs) -> Tuple:
    ...
    # 外部传入的scale（MM估计中应由S估计阶段传入）优先级最高
    if options["scale"]:
        scale = options["scale"]

    # ★ 关键：固定scale，整个M估计迭代过程中不再更新
    if scale < eps():
        scale = eps()
    fixed_scale = scale  # 明确命名，强调其不变性

    weights = (
        getRobustLocationWeights(resids / fixed_scale, options["weights"])
        * leverageWeights
    )

    # IRLS迭代：只更新params和weights，scale保持fixed_scale不变
    iteration = 0
    while iteration < options["maxiter"]:
        ...
        if np.sum(np.absolute(residsNew)) < eps():
            return paramsNew, residsNew, fixed_scale, weights

        # ★ 不再重新计算scale，始终使用fixed_scale
        weightsNew = (
            getRobustLocationWeights(residsNew / fixed_scale, options["weights"])
            * leverageWeights
        )
        ...
        # 收敛判断（基于残差变化，与scale无关）
        residsNorm = linalg.norm(residsNew)
        if residsNorm < eps():
            resids = residsNew
            break
        changeResids = linalg.norm(residsNew - resids) / residsNorm
        if changeResids < eps():
            resids = residsNew
            break
        resids = residsNew

    return params, resids, fixed_scale, weights  # 返回固定的scale

# def mestimateModel(A: np.ndarray, y: np.ndarray, **kwargs) -> Tuple:
#     r"""Mestimate robust least squares (with leverage down-weighting)
#
...
#         y = Ax .
#
#     Note: a pure M-estimator is robust against outliers in :math:`y` but not
#     against leverage points. This implementation multiplies the IRLS weights by
#     a leverage-based weight (bisquare on the hat-matrix diagonal), so it is in
#     fact a Mallows-type GM-estimator and offers some protection against
#     leverage points.
#
#     Parameters
#     ----------
...
#         Initial solution with parameters, residuals and (optionally) scale
...
#     options = parseKeywords(defaultDictionary(), kwargs, printkw=False)
#
#     # the design matrix used for regression must include the intercept column,
#     # and the leverage (hat-matrix diagonal) must be computed from that same
#     # matrix — otherwise the leverage weights are misaligned with the fit.
#     if options["intercept"] == True:
#         A = np.hstack((np.ones(shape=(A.shape[0], 1), dtype="complex"), A))
#
#     n = A.shape[0]
#
#     # leverage from the QR of the actual design matrix
#     q, _r = linalg.qr(A)
#     Pdiag = np.empty(shape=(n), dtype="float")
#     for i in range(0, n):
#         Pdiag[i] = np.absolute(np.sum(q[i, :] * np.conjugate(q[i, :]))).real
...
#         scale = sampleMAD0(resids)
#
#     # if an initial scale was provided, replace the one here
#     if options["scale"]:
#         scale = options["scale"]
#
#     # guard against scale collapse (residuals essentially zero)
#     if scale < eps():
#         scale = eps()
#
...
#         residsNew = y - np.dot(A, paramsNew)
#
#         # bail out if residuals are essentially zero (under/exactly determined)
#         if np.sum(np.absolute(residsNew)) < eps():
...
#         params = paramsNew
#
#         # convergence check on the relative change in residuals (R-style)
#         residsNorm = linalg.norm(residsNew)
#         if residsNorm < eps():
#             resids = residsNew
#             break
...
def getRobustLocationWeights_S(r: np.ndarray, weight: str) -> np.ndarray:
    """Robust weighting schemes
...
def solve_m_scale_bisquare(resids: np.ndarray, tol: float = 1e-6, max_iter: int = 50) -> float:
    ...
    对于bisquare, δ = 0.1995 对应50% breakdown point

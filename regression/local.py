...
from resistics.regression.compute import spectralMatrices, saveSpectralCSV, saveImpedanceCSV
...
from resistics.regression.robust import eps
...
class LocalRegressor(ResisticsBase):
    ...
    def process(self, ncores: int = 0) -> None:
        ...
        for declevel in range(0, numLevels):
            ...
            for eIdx in range(0, numEvalFreq):
                ...
                #####调用robustProcess
                # out, var, rms = self.robustProcess(numSolveWindows, obs, reg)
        ...
        # save and plot per-window weights (Usui et al. 2024, Figure 11 style)
        if len(self.weightData) > 0:
            self.saveAndPlotWeights()
    ...
    def robustProcess(
            self, numWindows: int, obs: np.ndarray, reg: np.ndarray, evalFreq: float
    ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Robust regression processing for a single evaluation frequency
        ...
        reg : np.ndarray
            The regressors
        evalFreq : float
            The evaluation frequency in Hz

        Returns
        -------
        output : np.ndarray
            The robust regression solution, shape (outSize, inSize)
        varOutput : np.ndarray
            The estimated variance, shape (outSize, inSize)
        """
        crossSize = len(self.crossChannels)
        ...
        for i in range(self.outSize):
            ...
            out, resids, scale, weights = mestimateModel(
            ...
            # #####⭐将权重值直接保存到文件中
            # # 创建保存目录
            # os.makedirs("weights_out_m", exist_ok=True)
            #
            # # 阻抗名称
            # impedance_names = ["x", "y"]
            ...
            #
            # # 文件名包含频率和阻抗
            # filename = f"weights_out_m/weights_{evalFreq:.6f}_{imp_name}_m_70_30.csv"
            #
            # df.to_csv(filename, index=False)
            # ################Z_polar_log_0.001381Hz_70_30_mm.pdf
            ...
            # variance estimation must use the same design matrix as mestimateModel
            if self.intercept:
                predFull = np.hstack((
                    np.ones(shape=(predictors.shape[0], 1), dtype="complex"),
                    predictors,
                ))
            else:
                predFull = predictors
            ...
            predWeighted = predFull * W
            varPred = np.dot(hermitianTranspose(predWeighted), predWeighted)
            try:
                varPredInv = np.linalg.inv(varPred)
            except np.linalg.LinAlgError:
                self.printWarning(
                    "Singular matrix at freq {:.6f} Hz, output channel {}".format(
                        evalFreq, self.outChannels[i]
                    )
                )
                varPredInv = np.eye(varPred.shape[0]) * 1e6

            varAll = 1.91472 * residsVar * varPredInv
            if self.intercept:
                varOut = np.diag(varAll).real[1:]
            else:
                varOut = np.diag(varAll).real
